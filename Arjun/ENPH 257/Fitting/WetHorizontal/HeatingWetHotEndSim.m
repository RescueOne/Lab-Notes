...
kwet = 22; %Constant to account for wet surface
e = 0.22; %Emmisivity of sandblasted Al rod
Cp = 857; %Specific heat capacity of Al (J/(K kg))
loss = .13; %Fractional loss of power to environment
...
%Temperature change due to wet part of rod (K)
dT_wet = @(Tx) (kwet*2*(Tx-Tamb)*dt)/(Cp*rho*a);
...
%initial
T(:,1) = ones(Nx,1)*(273+29.6); %Set all temperatures to Tamb
...
for time = 1:Nt-1
    ...
    %Temperature changes due to wet part of rod
    T(1:floor(Nx/3), time+1) = T(1:floor(Nx/3),...
        time+1) - dT_wet(T(1:floor(Nx/3), time+1));
...
Xsquare1 = sum(X1)%/1.1^2 %Sigma = 1.1
Xsquare2 = sum(X2)%/0.51^2 %Sigma = 0.51
Xsquare3 = sum(X3)%/2.8^2 %Sigma = 2.8
Xsquare4 = sum(X4)%/0.81^2 %Sigma = 0.81
